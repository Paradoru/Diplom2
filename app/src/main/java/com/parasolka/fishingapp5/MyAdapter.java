...
public class MyAdapter extends RecyclerView.Adapter<productviewholder> {
    ...
    public productviewholder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View mView = LayoutInflater.from(parent.getContext()).inflate(R.layout.insta_feed_rv_item, parent, false);
    ...
    public void onBindViewHolder(@NonNull productviewholder holder, int position) {
        ...
        mDataBaseRef.child(getFirebaseKey(holder.t4)).child("likes").addValueEventListener(new ValueEventListener() {
            @Override
        ...
        holder.b2.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (null != activenetwork) {
                    id3 = f1.getUid();
                    FirebaseDatabase.getInstance().getReference().child(getFirebaseKey(holder.t4)).child("likes").child(id3).setValue("1").addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
        ...
        holder.b3.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                NetworkInfo activenetwork = manager.getActiveNetworkInfo();
                if (null != activenetwork) {
                    FirebaseDatabase.getInstance().getReference().child(FirebaseAuth.getInstance().getUid() + "temporary").child("1").setValue(u).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
        ...
        holder.b1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                NetworkInfo activenetwork = manager.getActiveNetworkInfo();
                if (null != activenetwork) {
                    FirebaseDatabase.getInstance().getReference().child(id + "friends").child(holder.t5.getText().toString()).setValue(u2).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()) {
                                ...
                                FirebaseDatabase.getInstance().getReference().child(getFirebaseKey(holder.t5)).child("friends").child(id).setValue(u1);
    ...
    // Метод для замены недопустимых символов в ключе Firebase
    private String getFirebaseKey(TextView textView) {
