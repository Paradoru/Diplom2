...
public class MyAdapter4 extends RecyclerView.Adapter<productviewholder4> {
    ...
    public productviewholder4 onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View mView = LayoutInflater.from(parent.getContext()).inflate(R.layout.your_post_layout, parent, false);
        return new productviewholder4(mView);
    }

    @Override
    public void onBindViewHolder(@NonNull productviewholder4 holder, int position) {
        Glide.with(pcontext4).load(prolist4.get(position).getPropic()).into(holder.c1);
            ...
            public void onClick(View view) {
                ...
                if (null != activenetwork) {
                    ...
                    FirebaseDatabase.getInstance().getReference().child(holder.t4.getText().toString() + "likes")
                            .child("likes").child(id3).setValue("1").addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
        ...
        holder.b1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                NetworkInfo activenetwork = manager.getActiveNetworkInfo();
                if (null != activenetwork) {
                    FirebaseDatabase.getInstance().getReference().child("posts").child(holder.t4.getText().toString()).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            FirebaseDatabase.getInstance().getReference().child(FirebaseAuth.getInstance().getUid() + "posts").child(holder.t4.getText().toString()).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
